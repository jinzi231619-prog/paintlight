...
## 安静的展厅与可选音乐（2026-09-18）

界面保留随画作变化的冷暖色调，降低饱和度、收敛搜索框和按钮层级；中文标题使用系统衬线字体，不再等待第三方字体请求。

首次访问会邀请开启音乐，也可以安静浏览。顶部始终提供播放/暂停和曲目设置。每次加载页面默认静音，即使上次选择开启，也不自动播放；未点播放时，音频元素没有 `src`，不预加载录音。切换到后台会暂停，返回后手动恢复。邀请选择、曲目、音量仅存在 `paintlight-music-v1` 本地设置中；存储失败不影响暂停和浏览。

两首录音为 Kevin MacLeod 演奏/编配的萨蒂《第一号吉姆诺佩蒂》和帕赫贝尔《D 大调卡农》，CC BY 3.0，来源、许可与转码说明见 `public/audio/CREDITS.md` 和音乐设置。本站托管 MP3，只有播放时才传输。`music-tracks.mjs` 管曲目和设置，`music-player.mjs` 管单个播放器及异步竞争，`music.mjs`/`music.css` 管界面。

答题取消强制读取布局和整卡旋转，改为 90 ms 选择反馈与 160 ms 小幅淡入；进度点和观察标签按问题复用。关闭、撤回、跳过会取消待执行的切换，减少动态效果设置下不播放动画。相关逻辑与样式放在 `quiz-motion.mjs`、`classify.mjs`、`classify.css`。

自动验证覆盖默认无声、延迟播放期间暂停、快速换曲、播放失败重试、曲目循环、答题中途关闭/撤回及观察节点复用。性能优化来自减少同步布局与重复更新；不宣称未经真实设备测量的帧率提升。
